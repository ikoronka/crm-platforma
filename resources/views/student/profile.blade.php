...
@section('student-content')
    ...
    <form method="POST" action="{{ secure_url('student.profile.update') }}" class="mb-4" style="max-width: 600px;">
        @csrf
        @method('PUT')
    ...
    @if (Route::has('student.profile.destroy'))
        <form method="POST" action="{{ secure_url('student.profile.destroy') }}" class="mt-8">
            @csrf
            @method('DELETE')
